...
## 자기-개선 하니스 (Self-Harness 시각)

[[self-harness|Self-Harness]](Shanghai AI Lab, [[self-harness-paper]])는 *누가 하니스를 고치는가* 의 축을 끝까지 밀어붙인 세 번째 패러다임:

| 패러다임 | 하니스 개선 주체 |
|---|---|
| Human Harness Engineering | 인간 엔지니어 (이 연작의 기본 시각) |
| **Meta-Harness** | 더 강한 외부 에이전트 (위 [메타-하네스](#메타-하네스-managed-agents-시각) 절과 동류) |
| **Self-Harness** | **고정된 동일 모델이 자기 트레이스로 자기 하니스를 수정** |

- 이 연작의 핵심 명제 *"harness는 모델 약점에 대한 가정의 다발"* 을 **자동 루프로 구현** — 단 방향이 반대다. 이 페이지(Anthropic)는 *모델이 좋아지면 가정을 제거*(단순화)에 무게, Self-Harness는 *관찰된 실패에서 가정을 추가*(강화)를 모델 스스로 돌린다. 같은 진화 루프의 두 방향.
- 메커니즘: Weakness Mining(verifier-grounded 실패 클러스터링) → Harness Proposal(diverse yet minimal 후보) → Proposal Validation(held-in/held-out non-regressive gate). 위 *"future-proof 인터페이스"* 의 `harness`가 실제로 **자기수정 대상**이 된 형태.
- 증거: [[terminal-bench|Terminal-Bench-2.0]]에서 3개 모델([[minimax-m2-5]]·[[qwen3-5]]·[[glm-5]]) held-out pass rate가 일관 상승(최대 +21.4%p), 모델마다 *다른* edit이 채택됨 → *"harness는 inherently model-specific"* 을 정량 입증.
