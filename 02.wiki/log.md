...
## [2026-09-21] ingest | Tech Bridge — BM25는 왜 에이전트 검색에서 비정상적으로 효과적인가(Jo Bergum, Hornet) · 코딩 에이전트가 아니라 지식 에이전트로 설계하라(Benjamin Clavié, Mixedbread) (09-20 업로드 2편)

[[tech-bridge|Tech Bridge]] 최근 업로드 15편 중 **신규 2편**(둘 다 2026-09-20 업로드), 12편은 이미 ingest돼 있었다. **나머지 1편(`vMlsLmKuFZk`, 09-18 업로드, 37:03)은 멤버 전용이라 건너뛰었다** — 09-20에 확인한 그대로이고 **vault에 없으므로 dedup에 걸리지 않아 목록에 계속 나타난다**(가이드 §4a 목록을 먼저 보고 재판정하지 않았다). 전부 롱폼(Shorts 없음). ko·en-orig **429 없이 전부 확보**, 둘 다 `availability: public`.

**신규 source 2 · concept 16 · entity 6, 기존 11페이지 보강. index 실측 537 → 561.**

- [[tech-bridge-bm25-agentic-search]] (18:00, [[jo-bergum|Jo Kristian Bergum]] / [[hornet|Hornet]] CEO, 검색 경력 20년+) — **공식 챕터 11개**. **이 위키에 검색(IR) 전공자가 들어온 첫 소스**이고, 그 사실이 **위키가 깔고 있던 기본값을 드러낸다** — [[retrieval-augmented-generation|RAG]]·[[reference-graph-vs-vector-search]]·[[agent-collaboration-as-search]]가 전부 **벡터/임베딩을 전제**하고 있었고 **[[bm25|BM25]] 페이지조차 없었다**([[llm-wiki-pattern]]에 한 번 언급이 전부). 테제는 *"BM25는 변하지 않았다. 사용자가 바뀌었다"*(03:19~03:28) → [[llm-as-search-user]](AOL 로그의 몇 단어 vs 긴 쿼리·구문 연산자·재구성). **[[agentic-search]]** 가 검색을 **모델·하네스·엔진** 셋으로 쪼개 **실패의 책임을 분리 가능하게** 만들고, ⭐ **[[retrieval-not-reasoning-bottleneck]]** 이 그 분리를 실험으로 보인다 — *증거 문서를 컨텍스트에 채워 넣으면 정확도가 매우 높고*(∴ 추론은 병목이 아니다) *검색 도구가 달린 하네스를 주면 떨어진다*(06:55~07:34). **이것이 이 위키의 처방 순서를 바꾼다**: [[true-cost-to-perfect-answer]]·[[fixed-budget-alpha]]·[[token-roles]]·[[generator-evaluator-pattern]]이 전부 *더 생각하게 하라*였는데 **그 레버 앞에 "증거가 있었는가"가 온다**. **[[context-window-as-floppy-disk]]** 는 검색을 **[[harness-pruning]]의 소멸 경로에서 빼낸다**(*"완벽한 모델[AGI]을 얻어도 컨텍스트는 플로피 한 장"*) — 검색은 *모델이 못 하는 것에 대한 가정*이 아니라 **용량 제약에 대한 대응**이다. **[[corpus-as-filesystem-workspace]]**([[jimmy-lin|워털루 지미 린 그룹]] 논문)는 **09-19에 선 [[file-system-agent]]의 동적 판**이고 출처가 **제품이 아니라 논문**이며, [[agent-skills|스킬]]의 progressive disclosure를 **검색 결과에 적용**한다. **[[ride-the-optimization-trajectory]]** 가 이 위키가 [[file-system-agent]]·[[ralph-wiggum-method]]에서 **관측만 하고 이유를 대지 못했던 것**에 설명을 붙인다 — *"모든 최첨단 LLM 기업이 코딩·bash·도구 사용에 최적화 중이니 그 궤적에 얹으면 새 모델마다 좋아진다"*, **화자 스스로 "편법(hack)"** 이자 한시적 베팅이라 부른다. [[ir-evaluation-obsolescence]]는 **위키의 평가 축이 세 번 *더 잘게* 밀려 온 뒤 처음으로 방향을 뒤집는다**(더 크게 묶어서 재라). ⚠️ **발표 전체에 정확도 숫자가 0건**이고, 자사 비교는 **익명 상대 + 화자가 Y축을 QPS→지연으로 정정**(16:12~16:23)해 **이 위키는 인용하지 않았다.**
- [[tech-bridge-knowledge-agents-not-coding-agents]] (17:26, [[benjamin-clavie|Benjamin Clavié]] / [[mixedbread|Mixedbread]]) — **공식 챕터 10개**. **에이전트 아키텍처의 근거를 인류의 제도사에서 끌어온 첫 소스** — 알렉산드리아의 [피나케스]·수도원·대학·관료제·병원의 직급 분화([[tool-organization-loop]]). ⭐ **[[knowledge-work-agent-gap]](09-09, [[karan-vaidya|Vaidya]]/[[composio|Composio]])과 같은 현상에 두 번째 진단**을 댄다: 인프라의 결손이 아니라 **도메인과 과제 형태**다 → **[[code-as-atypical-knowledge]]**(*"지식 노동에는 **함수 정의가 없습니다**"*, *'30일'이 마감·유예기간·보존규칙으로 갈린다*). **그리고 결정적인 세 번째 겹** — *"[Claude Code]를 쓸 때 여러분이 그 작업을 직접 하고 있습니다. 이미 쿼리를 쪼개 놓았고 [Linear] 티켓을 갖고 있죠"*(12:06~12:19). **이 한 문장이 위키의 위임 논의를 재배치한다**: [[goal-level-delegation]]의 *"목표"조차 사람이 한 번 분해한 산출물*이고 [[plan-to-ticket-pipeline]]·[[sprint-contract]]·[[intent-md]]가 전부 그 분해 노동의 형태다. 처방은 **[[orchestrator-searcher-split]]**(로펌의 파트너/어시스턴트 — 메인은 쪼개고 서처가 조사해 **메모**를 올린다)이고, **이 위키에서 분업을 이득으로 셈한 첫 소스**라 [[agent-collaboration-as-search]](분업=손실)·[[agent-architecture-progression]](컨텍스트 손실 때문에 단순화)와 정면으로 놓인다 — ⚠️ **모순은 아니고 제약의 출처가 프라이버시냐 용량이냐가 다르다.** **[[oracle-gap]]**(완벽한 문서와 내 시스템의 차이)이 [[ir-evaluation-obsolescence]]가 남긴 *책임 소재* 공백을 메우고, [[tools-are-not-neutral]]은 *"25번 호출이면 아무도 안 쓰고 8번이면 도입한다"* 로 **정확도가 같은데 채택이 갈리는** 자리를 잡는다. [[retrieval-primitive-repertoire]]는 위키가 처음 받는 도구 실패 모드다 — **학습 데이터 분포가 도구 선택을 편향시킨다**(*"PDF는 `grep` 할 수 없습니다"*). ✅ **자기 한계를 먼저 말한다** — *"왜 내 에이전트는 88.9인데 인간은 99.4죠? 그런데 저는 그걸 이해할 수 없습니다"*(13:44~13:58).

**⭐ 채널 첫 상호 참조 — 두 영상이 자막 안에서 서로를 지목한다.** Clavié가 *"오늘 아침에 이 얘기를 하셨는데"*(09:23)로 [[browsecomp-plus|BrowseComp-Plus]]를 다룬 Bergum 편을 가리키고, Bergum은 *"[Ido]가 코드 모드를 시연했다"*(01:54)·*"[Edu]가 일반 상식 이야기를 했다"*(03:28)로 같은 무대의 다른 발표를 가리킨다. **같은 날 같은 무대의 연속 발표임이 상호 확인된다.** ⚠️ **그런데 행사 이름은 두 설명란 모두에 없다** — 09-20 [[tech-bridge-rlhf-assistance-vs-automation|Almeida 편]]은 설명란이 *AI Engineer World's Fair* 를 명시했는데 이번 둘은 그렇지 않아 [[ai-engineer]] 계열인지 **확정하지 않았다.** [Ido]·[Edu]는 **성이 없어 인물을 확정하지 않았다.**

**⭐ 세 자리에서 독립 수렴, 한 자리에서 정면 충돌.** ① **[[which-bm25-problem]]** — Bergum *"어떤 BM25를 말씀하시는 건가요?"*(10:25) vs Clavié *"BM25는 하나가 아닙니다. 수백 개나 됩니다"*(09:43). **둘 다 자기 제품을 파는 자리에서, 둘 다 자기에게 유리하지 않은 방향으로 같은 방법론적 결함을 지목한다** — 이 페이지가 위키의 *"X가 Y보다 낫다"* 형 주장 전체([[reference-graph-vs-vector-search]]·[[self-harness]]·[[slop-probes]])에 붙는 각주가 된다. ② **[[oracle-gap]]** — Bergum이 실험으로 보인 양(정성)에 Clavié가 이름과 숫자를 붙인다(정량, ⚠️ 자기 보고). ③ **컨텍스트를 늘려도 안 된다** — 근거가 갈린다(품질 저하 vs 비용+절대 규모), [[long-context-agents]]와 축이 다르다. **⚠️ 충돌**: 모델이 `grep`·BM25에 익숙하다는 **같은 사실**이 Bergum에게는 [[ride-the-optimization-trajectory|레버]]이고 Clavié에게는 [[retrieval-primitive-repertoire|편향]]이다 — **갈리는 조건은 도메인(텍스트 vs 스캔 PDF)인데 두 소스 어느 쪽도 그 구분을 명시하지 않는다. 이 위키가 겹쳐서 읽고 조건만 기록했다.**

**⭐ 위키가 반대 방향에서 이미 같은 선을 그어 두고 있었다.** [[code-as-atypical-knowledge]]가 코드/비코드를 검색 문제로서 가르는데, [[reference-graph-vs-vector-search]](09-15, *"자연어에서는 유사도가 곧 관련성인데 코드에서는 반대말이 가장 비슷하게 생긴다"*)와 [[code-only-index-blind-spot]]이 **서로 모른 채 같은 선을 두 각도에서** 그어 놓았다. **세 출처가 다른 회사·다른 이해관계로 같은 구분에 도달한다.**

**자막 관찰 — 고유명사 직역이 이번 회차의 최대 오류.** ① ⚠️ **회사 이름 두 개가 한 문장에서 통째로 녹았다** — *"something that **hugging face and snowflake** jointly released"* → ko **"얼굴을 껴안고 눈송이처럼 섬세한 것"**(12:38). **벤치마크의 출처를 말하는 자리다.** [[hugging-face]] 직역은 09-20에도 있었으나 그때는 한 회사였다. ② ⚠️ **`grep`이라는 도구 이름이 ko에서 사라진다** — **en-orig 자체가 전편에서 *"grape"*** 로 적고 ko가 영어를 그대로 두거나 *"파악"* 으로 의역. **Clavié 편의 핵심 논거가 통째로 이 단어 위에 있다.** ③ ⚠️ **LLM 오역 사흘 연속, 이번엔 테제 문장에서** — *"The **LLMs** today have a lot of general knowledge"* → **"법학 석사(LLM) 과정 학생들"**(Bergum 03:31). ④ ⚠️ **en-orig 오인식을 ko가 직역하는 연쇄 다섯 건** — *betting models*→임베딩 모델 · *party goals*→패러리걸(ko *"정당의 목표"*) · *double cues*→durable cues · *sales of tools*→set of tools · *savings*→ceilings. ⑤ ⚠️ **숫자가 en-orig에서 깨진 자리 둘** — *"820 30 questions"*(→830, **설명란·챕터가 확정**) · *"889.8"*(→89.8). ⑥ ⚠️ **없던 업종 창작** — *"partner of the **agency**"* → **"광고 대행사의 파트너인 변호사"**(단락 전체가 로펌). ⑦ ⚠️ **단위가 바뀜** — *"25 **calls**"* → **"25번의 통화"**(툴 호출 단락). ⑧ ⚠️ *Claude Code* → *cloud code* → **"클라우드 코드"**, **이틀 연속 재발.** ⑨ **한 벤치마크가 한 영상에서 네 갈래 표기**(BrowseComp-Plus). ⑩ *"Right?"* → *"오른쪽?"* **사흘 연속.**

**✅/⚠️ 설명란에 대한 관측이 갈렸다.** ✅ **이름을 또 두 번 확정했다** — **Jo Kristian Bergum**(양 트랙 *"Joe Bergum"*)·**Benjamin Clavié**(양 트랙 *"Ben Clavier"*). **채널 일곱 번 연속 설명란 의존.** ⚠️ **그런데 수치는 설명란이 자막보다 세게 적었다** — 설명란 *"오라클 갭을 **40% 이상**"* vs 자막 *"**약** 40%"*(14:53), 그리고 설명란이 *20% 적은 호출*을 *"단순 BM25 대비 멀티모달 검색"* 대비로 적었으나 **자막에 그 대응이 없다.** **이 위키는 자막을 따랐다.** → **운영 교훈: 설명란은 고유명사에는 신뢰하되 수치에는 같은 신뢰를 주지 않는다.**

**해소하지 않고 표시만 한 것**: **행사 이름**(두 편 다) · **촬영 시점**(Bergum 편의 월드컵·펍 퀴즈 언급은 강한 앵커지만 대회·연도를 말하지 않아 추정하지 않음) · [[browsecomp-plus]] **코퍼스 크기 세 갈래**(10만5천 / 20만 / 10만, **Clavié는 자기 발표 안에서 두 번 다르게 말한다**) · **벤치마크 이름 판독 불가**(*"[MQA]"*) · *"Ginai"*(→Gemini 추정) · [Ido]·[Edu]·[[jimmy-lin|Jimmy Lin]]의 확정 · **BM25 두 하이퍼파라미터의 이름**(소스에 없어 위키도 쓰지 않음) · **하이브리드를 어떻게 섞는가** · Clavié 편 정확도 수치(60/70~80/90/89.8/90.2)가 **어느 시스템인지** · **서처의 개수·모델·비용** · **오라클의 정의** · **남는 6포인트의 원인** · [[hornet]]·[[mixedbread]]의 규모·자금·가격 · ⚠️ **두 편 다 보안·권한·프롬프트 인젝션 0건**([[lethal-trifecta]]가 경고한 조합이 장점으로만 제시됨 — **09-19 Vercel 편과 같은 공백의 재발**) · ⚠️ **Clavié 편에 검증·승인 게이트가 없다**(법률·의료를 예시로 드는 발표인데 [[risk-proportional-human-review]]에 해당하는 것이 없고, [[agent-collaboration-as-search]]가 기록한 *"정보 하나가 틀리면 영원히 오염된다"* 가 서처의 메모에 그대로 적용되는데 다루지 않는다) · ⚠️ [[tool-organization-loop]]에 **분업의 비용 항이 없다.**

**운영 메모**: 09-18 지침대로 **목록을 파일로 받고 `wc -l`로 15편 확인** — **이번 회차에 그 지침이 실제로 값을 했다.** 첫 시도에서 `tail`로 목록을 자르는 바람에 **최신 3편(09-20 업로드 2편 포함)이 통째로 가려졌고**, 파일로 다시 받아 `wc -l` 한 뒤에야 드러났다. **WARNING 줄이 섞이는 한 `head`/`tail`은 쓰지 않는다.** 자막은 `--sub-format json3`으로 받아 dedup 후 **개별 자막 이벤트 시각**을 인용 타임스탬프로 썼다. 두 편 다 **공식 챕터를 따랐다**(11개·10개) — 이번엔 챕터와 내용이 어긋난 자리가 없었다. `--cookies-from-browser chrome` 로 **429 없이** 네 트랙 전부 확보. yt-dlp의 *impersonation target 없음* 경고가 났으나 자막·메타 모두 정상 반환.


## [2026-09-22] ingest | Tech Bridge — AI 에이전트를 위한 완벽한 검색 (Will Bryk, Exa) (09-21 업로드 1편)

`--playlist-end 15` 가 **15편** 반환(전부 롱폼, 최단 531초, Shorts 없음). 09-21 업로드 **3편** 중 **1편만 ingest 가능**했다 — 12편은 기존, **2편이 멤버 전용**(`vMlsLmKuFZk`는 §4a 목록대로 재판정 없이 건너뜀, `XZuws4hFG4o`는 **신규 판정**). ko·en-orig 확보, **429 없음**. `en`과 `en-orig`는 **바이트 단위로 동일**.

**신규 source 1 · concept 7 · entity 2**, 기존 **15페이지 보강**, index 실측 **561 → 571**.

`X66cWPrmiq4`(17:13, **Will Bryk / Exa** — AI 에이전트를 위한 완벽한 검색) → [[tech-bridge-exa-perfect-search-for-agents]]

### ⭐ 이번 실행의 구도 — 하루 만에 온 반대편

**전날 ingest한 [[tech-bridge-bm25-agentic-search|Bergum 편]]([[hornet|Hornet]])과 같은 질문에 반대로 답한다.** 두 사람은 서로를 모르고, 각자 자기 제품의 방향으로 답하며, **둘 다 정확도 수치가 0개**다.

| | [[jo-bergum\|Bergum]] · [[hornet]] | [[will-bryk\|Bryk]] · [[exa]] |
|---|---|---|
| 테제 | BM25는 변하지 않았다, **사용자가 바뀌었다** | **키워드로는 복잡한 쿼리를 감당할 수 없다** |
| 어휘 검색 | **저평가된 프리미티브** | *"효율적"* 이지만 **간단한 쿼리까지** |
| 임베딩 | 비용 항목 | **핵심**([[sutton-bitter-lesson]]) |
| 병목 | **검색**이지 추론이 아니다 | **비용** |
| 평가 | nDCG는 끝났다 | ⚠️ **말하지 않는다** |

**⭐ 만나는 지점이 한 줄 있고 그것을 말하는 쪽이 임베딩 진영이다** — *"임베딩과 키워드를 함께 사용하는 경우가 많습니다"*(06:58). [[bm25]]가 남긴 미해결(*하이브리드를 어떻게 섞는가*)에서 **세 번째 벤더도 똑같이 멈춘다.**

**무엇이 사라졌는가** (09-18 재방문 규칙):
- **정확 일치**(Bergum의 ①)에 **반박하지 않고 닿지도 않는다** — 그런데 대표 데모가 *YC 투자 스타트업의 배치와 상태*·*가장 많이 인용된 논문과 졸업 연도* 로 **전부 정확 일치의 영역**이다.
- **설명 가능성**(③ — 모델이 왜 그 결과가 나왔는지 알아야 다음 쿼리를 쓴다)이 없다.
- ⚠️ **궤적이 통째로 없다.** [[agentic-search]]의 *쿼리 → 읽기 → 재구성* 루프가 **한 번도 나오지 않고**, 에이전트는 **답을 받아 가는 소비자**로 그려진다.

→ **제품 모양의 차이로 보인다**(반복해 두드리는 **프리미티브** ↔ 한 번에 주는 **완성된 결과**). **두 소스 어느 쪽도 이 구분을 말하지 않는다 — 이 위키가 겹쳐서 읽은 것이다.**

### 새로 들어온 축 — 목적함수

[[search-as-recommendation-engine]]: *"[구글은] 세상의 정보를 담은 데이터베이스가 되려는 게 아닙니다. 그건 일종의 추천 엔진이에요"*(02:58~03:05). **이 위키가 검색 엔진을 *무엇을 최적화하도록 만들어졌는가* 로 본 첫 자리**이고, 이틀 전 [[preference-reward-asymmetry]](*과대약속은 버그가 아니라 특징*)와 **같은 모양의 논증**이다. ⚠️ 다만 [[diogo-almeida|Almeida]]는 메커니즘을 명시했고 **이쪽은 비유에서 멈춘다.**

⭐ [[suppressed-query-demand]] — *"이런 걸 구글에 쳐 본 적이 없을 겁니다. **안 될 걸 아니까요**"*(03:12). **화자도 그냥 지나가는 한 줄인데**, [[llm-as-search-user]]의 AOL 로그 관측에 **두 번째 읽기**를 연다: *사용자가 바뀌었다* 는 **제약이 풀렸다** 로도 읽힌다. ⚠️ **두 읽기 다 증거가 없다.**

### 나머지 넷은 기존 페이지와 충돌하거나 정반대다

- [[perfect-search-as-cost-problem]] — 쌍마다 LLM을 돌리면 거의 완벽하지만 **검색당 1천만 달러**, 따라서 *10억~1조 배 줄이는 최적화 문제*. **품질을 비용으로 재정의한 세 번째이자 가장 극단**([[fixed-budget-alpha]]·[[true-cost-to-perfect-answer]]가 *이미 돌아가는 에이전트의 지출* 이었던 것과 달리 **이상적 해가 먼저 정의된다**). ⚠️ [[context-window-as-floppy-disk]]와 **정면으로 만나지만 축이 다르다** — 후보를 1조 개에서 10개로 줄여도 **그 10개를 창에 넣는 문제는 남는다.** 이 위키는 어느 쪽도 채택하지 않았다.
- [[retrieval-side-context-compression]] — 문서 10개에서 **가장 중요한 100 토큰만**. **압축을 검색 엔진이 대신 하는 첫 형태**이고, [[corpus-as-filesystem-workspace|전부 줘라]]·[[bm25|왜 나왔는지 알려 줘라]]와 함께 **나흘 사이 같은 문제에 세 처방**이 됐다. ⚠️ **세 번째는 앞의 둘과 양립하기 어렵다.**
- [[per-customer-search-engine]] — *고객 5,000곳에 각각 다른 엔진*. [[which-bm25-problem]]이 **결함**으로 세운 가변성을 **기능**으로 내놓고, 그 결과 같은 발표의 *"구글보다 낫다"* 가 **원리적으로 검증 불가능**해진다. **화자는 장점으로만 제시한다.**
- [[agent-data-marketplace]] — **데이터 유통·정산이 위키에 처음.** ⚠️ *"에이전트는 출처를 신경 쓰지 않는다"* 가 **여기서는 장점인데 [[lethal-trifecta]]·[[prompt-injection]]은 정확히 그 무관심을 공격 조건으로 본다.**
- [[search-latency-tiers]] — 09-19 [[voice-latency-thinking-tradeoff]]에 **공급자 쪽 짝**(200ms · *"인간에게는 너무 빠르다"*). ⚠️ **09-19에 세운 질문은 여전히 답을 못 받았다** — 음성 예산에서 검색의 몫은 양쪽 어디에도 없다.

### ⚠️ 보안이 17분 동안 한 번도 나오지 않는다

신뢰할 수 없는 웹 문서와 **유료 비공개 데이터**를 섞어 에이전트 컨텍스트로 밀어 넣는 파이프인데 [[prompt-injection]]·[[lethal-trifecta]]·신뢰 경계·출처 검증이 **전무하다.** **검색·retrieval을 파는 소스에서 연속 세 번째**(09-19 Vercel · 09-21 Bergum · 여기).

### ⭐ 운영 — 멤버 전용 영상이 두 편이 됐다

09-21 업로드 `XZuws4hFG4o`(*엔비디아 CEO 젠슨 황*, 46:18)이 **`availability: subscriber_only`** 로 판정됐다(`No video formats found` · `automatic_captions` 빈 배열 · *"Join this channel…"* 경고). 09-18의 `vMlsLmKuFZk`에 이은 **두 번째**다.

> **이제 일회성 사고가 아니라 채널의 운영 패턴으로 봐야 한다** — 두 편 다 **유명 인물 대담**(샘 올트먼×베니오프 · 젠슨 황)이고 **롱폼 중에서도 긴 쪽**(37:03 · 46:18)이다. **이 위키는 두 편 모두 페이지를 만들지 않았다.** → 가이드 §4a 제외 목록에 `XZuws4hFG4o` 추가.

### 자막 관찰 — 발표 주인공의 회사 이름이 네 갈래로 깨졌다

- ⚠️ **이번 회차 최대 오류** — *Exa* → **Exxon**(01:22) · **Exo**(01:47·07:38·15:16) · **exon**(13:22) · **XA**(13:02·14:26). **발표 전체가 그 회사 이야기인데 본문에서 이름이 한 번도 안정적으로 나오지 않는다.** 제목·설명란·챕터만 옳다. 09-19 원칙의 재적용인데 **이번엔 어긋난 것이 주제어가 아니라 화자 자신의 회사명**이다.
- ⚠️ **없던 도메인 창작 — 회사가 밴드로** — *"we were called Metaphor at the time"* → **"저희 밴드 이름이 메타포였어요"**(07:44), 앞 문장도 **"저희가 밴드를 시작한 지"**. 그리고 *the [Exa] team*(ASR *exit team*) → **"퇴사팀"**(08:04). **창업 서사 전체가 다른 업종으로 옮겨 간다.**
- ⚠️ **수식어가 옮겨 붙어 주장이 부풀었다** — *"we launched **our first** search engine"* → **"세계 최초로 검색 엔진을 출시"**(07:48). 09-19 *한정어 소실로 모수 확대* 의 **거울상**이다(이번엔 한정어가 **더 강한 자리로 이동**).
- ⚠️ **수량이 5,000배로** — *"5,000 search engines for **each of** our 5,000 customers"* → **"고객 5,000명당 각각 5,000개"**(12:34).
- ⚠️ **LLM에 없던 확장을 창작, 이번엔 괄호까지** — *LM*·*LLM* → **"로지스틱 맵"**(11:45·11:58). 09-19 *Learning Leadership Model* · 09-20 *법학도/LMS* 에 이은 **나흘째 재발**.
- ⚠️ **agent가 한 영상에서 네 갈래** — **상담원**(11:19) · **요원**(13:24) · **대행사**(01:24) · **설계사**(01:31). 09-18·09-19의 재발인데 **한 영상 안에서 네 번은 처음**이다.
- ⚠️ **회사명이 보통명사로 녹아 예시가 무너짐** — *SimilarWeb* → **"유사 웹사이트"**(14:13·14:20). **마켓플레이스 데모의 유일한 구체 예시**다.
- ⚠️ 기술 용어가 사람 집단으로 — *queries* → **"퀴어들"**(07:06). 관용구가 정치 용어로 — *left field* → **"좌익적인 행보"**(07:19). 검색 속도가 배송 속도로 — *"Do you want super fast?"* → **"초고속 배송"**(12:47). *saved*(구원받았다) → **"데이터가 저장"**(08:09, 회사 전환점을 여는 문장). *"So instead of a world like this, no[,] bad"* → **"나쁜 일은 하나도 없을 거예요"**(14:37, 디스토피아 슬라이드를 가리키는 자리인데 뜻이 뒤집혔다).
- ✅ **설명란이 이름을 또 확정했다** — **Will Bryk**. 이번엔 **자막에 이름이 아예 없다**(자기소개 없는 발표). **채널 여덟 번 연속 설명란 의존.**
- ✅ **촬영 연도가 두 앵커로 교차 검증된 첫 사례** — *"지금은 2026년"*(00:33) + *"창업 2021년, 어제가 5주년"*(06:14~06:18). 09-20에 단일 앵커로 나온 유형의 **강화판**. 월·일은 미확정.
- **판독하지 않은 것**: **13:22의 *"exon"*** — *"최근 출시한 것"* 자리라 별개 제품명일 수 있으나 **어느 트랙도 풀지 않고 설명란도 주지 않아 이름을 만들지 않았다.** 기능(비공개 데이터 마켓플레이스)으로만 적었다.
- 양 트랙이 함께 깨져 **문맥이 명백한 것만 교정해 읽음**: **GB3/GB5/GB6/GB7 → GPT-3/5/6/7**, **CHPT → ChatGPT**, **"샌 이스코" → San Francisco**, **"LMS용 AIS" → LLM용 AI**.

### 해소하지 않고 표시만 한 것

**행사 이름** · 촬영 **월·일** · 화자의 정확한 **직함**(설명란은 *"창업자"* 까지) · ⚠️ **검색량 역전 그래프의 출처·방법·"1회 검색"의 정의**(발표가 *"단 하나만 기억하라"* 고 지목한 수치인데 근거가 비어 있다) · ⚠️ **정확도 수치 0개**(*"구글보다 낫다"*·*"세계에서 가장 빠른"*·*"최고 품질"*) · **200ms와 "몇 분"을 가르는 메커니즘** · **비용을 10억~1조 배 줄였다는 주장의 실제 경로**(*"다양한 시스템의 결합"* 으로 닫는다) · **마켓플레이스의 가격·정산·라이선스·감사·출처 표기** · [[exa|Exa]]의 **자금·인원·가격·수익** · ⚠️ **평가론 전무**(하루 전 [[ir-evaluation-obsolescence]]가 선 자리가 비어 있다) · **[[cursor|Cursor]]가 Exa를 쓴다는 것은 [[will-bryk|Bryk]] 쪽 진술이고 Cursor의 확인이 아니다.**
