...
2. **Claude Code를 이 디렉토리에서 실행** — `CLAUDE.md`를 읽어 위키 운영 규칙 학습
3. **소스 추가**: `01.raw/`에 파일을 넣고 `/ingest` 호출
4. **책 추가**: `/reading-add` 로 to-read 등록
5. **TIL 기록**: `/til` 로 오늘 배운 것 한 줄 정리
6. **질문**: `/query <질문>` 또는 자연어로 ("위키에서 X 찾아줘") — 관련 페이지 정독 후 `[[인용]]` 포함 답변
7. **점검**: `/lint` 로 모순·고아 페이지·독서 상태 불일치 점검
